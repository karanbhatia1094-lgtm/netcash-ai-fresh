...
export default function CampaignsPage() {
  ...
  const filteredSourceOptions = sources.filter((item) => String(item || "").toLowerCase().includes(sourceSearch.toLowerCase()));
      ...
      <div className="nc-card nc-section nc-glass nc-campaign-controls">
        ...
        <div className="nc-campaign-controls-grid">
          ...
          <div className="nc-campaign-control-group">
            ...
            <details className="nc-source-picker" open={sourcePickerOpen}>
              <summary onClick={(event) => {
                event.preventDefault();
                setSourcePickerOpen((current) => !current);
              }}>
                {isAllSources ? "All sources" : `${activeSources.length} selected`}
              </summary>
              {sourcePickerOpen ? (
                <div className="nc-source-picker-panel">
                  ...
                  />
                  <button type="button" className="nc-chip" onClick={() => toggleSource("all")}>All</button>
                  {filteredSourceOptions.map((item) => (
                    <button
                      key={`source-picker-${item}`}
                      type="button"
