...
const STYLE_VERSION = "nav-2026-03-21-3";
export const links = () => [{ rel: "stylesheet", href: `${netcashStyles}${netcashStyles.includes("?") ? "&" : "?"}v=${STYLE_VERSION}` }];
...
export const loader = async ({ request }) => {
  ...
  const isHomeRoute = url.pathname === "/app";
  const userAgent = String(request.headers.get("user-agent") || "");
  const isMobile =
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(userAgent);
  const allowMobileHomePreview = isHomeRoute && isMobile;
  if (isProduction && !isBillingRoute && !allowMobileHomePreview) {
...
export default function App() {
  ...
  const navItems = [
    { to: "/app", label: "Home", shortLabel: "Home", icon: "home" },
    { to: "/app/campaigns", label: "Campaigns", shortLabel: "Camp", icon: "campaigns" },
    { to: "/app/universal", label: "Universal Insights", shortLabel: "U-Ins", icon: "insights" },
    { to: "/app/intelligence", label: "Intelligence", shortLabel: "Intel", icon: "intelligence" },
  ];
          ...
          <nav className="nc-app-topnav" aria-label="Workspace">
            {navItems.map((item) => (
              ...
              >
                ...
                <span className="nc-nav-label">
                  <span className="nc-nav-label-full">{item.label}</span>
                  <span className="nc-nav-label-short">{item.shortLabel || item.label}</span>
                </span>
              </NavLink>
            ))}
          </nav>
